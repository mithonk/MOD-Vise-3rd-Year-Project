    ...
    datasets: [
      {
        ...
        data: [
          ...
          orders.filter(
            (o) => new Date(o.createdAt) > subHours(new Date(), 24 * 21)
          ).length,
          orders.filter(
            (o) => new Date(o.createdAt) > subHours(new Date(), 24 * 14)
          ).length,
          orders.filter(
            (o) => new Date(o.createdAt) > subHours(new Date(), 24 * 7)
          ).length,
  ...
  return (
    <div>
      ...
      <div className="tiles-grid">
        <div className="tile">
          <h3 className="tile-header">Today</h3>
          <div className="tile-number">Rs {ordersTotal(ordersToday)}</div>
          <div className="tile-desc">{ordersToday.length} orders today</div>
        </div>
        <div className="tile">
          <h3 className="tile-header">This week</h3>
          <div className="tile-number">Rs {ordersTotal(ordersWeek)}</div>
          <div className="tile-desc">{ordersWeek.length} orders this week</div>
        </div>
        <div className="tile">
          <h3 className="tile-header">This month</h3>
          <div className="tile-number">Rs {ordersTotal(ordersMonth)}</div>
          <div className="tile-desc">
            {ordersMonth.length} orders this month
          </div>
        </div>
